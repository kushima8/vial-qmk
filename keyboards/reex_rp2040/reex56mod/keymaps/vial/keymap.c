...
// When a side has no trackball, its SPI pins (GP18/GP19/GP20) can be used
// for the rotary encoder and the DIP switch instead.  SPI initialization
// runs before this hook, so re-initialize the encoder and the DIP switch to
// reclaim those pins on trackball-less sides.
#if defined(ENCODER_ENABLE) && defined(DIP_SWITCH_ENABLE)
static void reclaim_encoder_pins(void) {
...
#if defined(ENCODER_ENABLE) && defined(DIP_SWITCH_ENABLE)
    // On the master, wait until the split negotiation finished before
    // touching the pins.
    static bool encoder_reclaimed = false;
    if (!encoder_reclaimed && reex.negotiated && is_keyboard_master()) {
        reclaim_encoder_pins();
        encoder_reclaimed = true;
    }
#endif
}
