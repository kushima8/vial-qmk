...
この機能は config.h に `#define GBC_SCROLLSNAP_ENABLE 0` を書き加えることで無効化できる。
...
この振る舞いは config.h に `#define GBC_SCROLLSNAP_ENABLE 1` を書き加えることで有効化できる。
...
フリーモードに遷移するためのカウント数を変更するには `GBC_SCROLLSNAP_TENSION_THRESHOLD` を、
スナップモードに遷移するためのインターバル(ミリ秒)を変更するには `GBC_SCROLLSNAP_RESET_TIMER` を、それぞれ config.h で設定できる。

以下はカウント数を 5 に、インターバルを 200 ミリ秒に変更する例:

```c
#define GBC_SCROLLSNAP_TENSION_THRESHOLD 5
#define GBC_SCROLLSNAP_RESET_TIMER 200
```
...
この値は config.h で `GBC_SCROLL_DIV_DEFAULT` マクロを定義することで変更できる。
これを0にすることは考慮していないので設定しないこと。

### Scroll Inhivitor

トラックボールの移動量をポインタに適用するかスクロールに適用するか、
GBCは内部にスクロールモードという名のモードで管理している。
...
Scroll Inhivitor は config.h で `GBC_SCROLLBALL_INHIVITOR` マクロを定義することで変更できる。
